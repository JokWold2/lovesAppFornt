...
test('the interaction conversation badge uses the full unread summary and clears independently of profile views', async () => {
  ...
  const context = { module: { exports: {} }, ref: value => ({ value }), computed: fn => ({ get value() { return fn() } }),
    watch: () => {}, onShow: () => {}, onHide: () => {}, onUnload: () => {}, currentLocale: {},
    uni: { getStorageSync: () => ({}) }, getToken: () => 'session', console,
  ...
  runInNewContext(code + '\nmodule.exports = {load, interactionUnread, badgeText}', context)
