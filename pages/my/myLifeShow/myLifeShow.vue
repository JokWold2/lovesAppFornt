...
<view class="my-trade-links"><view v-for="r in ['buying','selling','bidding']" :key="r" @click="openMyTrades(r)"><uni-icons type="shop" size="20" color="var(--bless-primary, #C2A052)"/><text>{{ t('trade.'+r) }}</text></view></view>
    <view v-show="activeTab === 'moments'" class="moments-pane" role="tabpanel">
      <button class="moments-composer" @click="goToEdit">
        <view class="composer-symbol"><uni-icons type="compose" size="24" color="var(--bless-text, #775E25)" /></view>
      ...
      <view v-if="loading && !moments.length" class="hub-state"><view class="state-icon"><uni-icons type="more-filled" size="28" color="var(--bless-text, #775E25)" /></view><text>{{ t('momentsHub.loading') }}</text></view>
      <view v-else-if="momentsError" class="hub-state error-state"><view class="state-icon"><uni-icons type="reload" size="26" color="var(--bless-text, #775E25)" /></view><text>{{ t('momentsHub.momentsError') }}</text><button class="state-button moments-retry" @click="loadMoments">{{ t('momentsHub.retry') }}</button></view>
      <view v-else-if="!moments.length" class="hub-state empty-state"><view class="state-icon"><uni-icons type="compose" size="28" color="var(--bless-text, #775E25)" /></view><text class="state-title">{{ t('momentsHub.emptyTitle') }}</text><text class="state-hint">{{ t('momentsHub.emptyHint') }}</text><button class="state-button" @click="goToEdit">{{ t('momentsHub.publish') }}</button></view>
      <view v-for="item in moments" :key="item.id" class="post-card" @longpress="showItemActions(item)">
        ...
        <view class="post-actions">
          <button class="post-like-button" :class="{ 'is-liked': item.is_liked }" :disabled="item.likePending" :aria-label="t('momentsHub.likeAction')" :aria-pressed="!!item.is_liked" @click.stop="toggleLike(item)"><uni-icons :type="item.is_liked ? 'heart-filled' : 'heart'" size="23" :color="item.is_liked ? 'var(--bless-primary, #C2A052)' : '#7e8677'" /><text>{{ item.like_count || 0 }}</text></button>
    ...
    <view v-show="activeTab === 'profile'" class="profile-pane" role="tabpanel">
      <button v-if="!receivedLikesError" class="profile-likes-entry" @click="goToLikes"><uni-icons type="heart" size="24" color="var(--bless-primary, #C2A052)" /><text class="profile-likes-label">{{ t('momentsHub.profileLikes', { count: receivedLikeTotal }) }}</text><uni-icons type="right" size="17" color="#96917b" /></button>
