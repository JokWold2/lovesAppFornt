<template>
              ...
              <view class="day-group">
                <view class="td sticky-col-1 col-date date-cell day-date-cell">
                  <view class="date-content">
                    <picker mode="date" @change="(e) => onDateChange(e, index)">
                      <!-- 占位文字：字号12px，颜色浅灰 -->
                      <text
                        :class="dayItem.date ? 'date-text' : 'placeholder-text'"
                ...
                <view class="day-rows">
                  ...
                  <view class="tr">
                    ...
                    <view class="td col-income"><input type="digit" placeholder="请输入金额" v-model="dayItem.morning.income" placeholder-class="ph-color" /></view>
                    <view class="td col-cash"><input type="digit" placeholder="请输入金额" v-model="dayItem.morning.cash" placeholder-class="ph-color" /></view>
                    <view class="td col-octopus"><input type="digit" placeholder="请输入金额" v-model="dayItem.morning.octopus" placeholder-class="ph-color" /></view>
                  ...
                  <view class="tr">
                    ...
                    <view class="td col-income"><input type="digit" placeholder="请输入金额" v-model="dayItem.noon.income" placeholder-class="ph-color" /></view>
                    <view class="td col-cash"><input type="digit" placeholder="请输入金额" v-model="dayItem.noon.cash" placeholder-class="ph-color" /></view>
                    <view class="td col-octopus"><input type="digit" placeholder="请输入金额" v-model="dayItem.noon.octopus" placeholder-class="ph-color" /></view>
                  ...
                  <view class="tr">
                    ...
                    <view class="td col-income"><input type="digit" placeholder="请输入金额" v-model="dayItem.night.income" placeholder-class="ph-color" /></view>
                    <view class="td col-cash"><input type="digit" placeholder="请输入金额" v-model="dayItem.night.cash" placeholder-class="ph-color" /></view>
                    <view class="td col-octopus"><input type="digit" placeholder="请输入金额" v-model="dayItem.night.octopus" placeholder-class="ph-color" /></view>
  ...
  onLoad((options) => {
    if (options.title) {
      ...
      if (title.includes('财务')) {
        uni.redirectTo({
          url: '/pages/index/components/financialTable'
        })
      }
    }
  })
  ...
  const tableData = ref(Array.from({ length: 10 }, () => createEmptyDay()))
  ...
  const handleSubmit = async () => {
    ...
    try {
      const now = new Date()
      const title = `${now.getFullYear()}年${now.getMonth() + 1}月报销明细表`
  ...
  .col-shift { width: 60px; }
  .col-income { width: 120px; }
  .col-cash { width: 120px; }
  .col-octopus { width: 120px; }
  ...
  .sticky-col-2 {
    position: sticky;
    left: 100px;  /* 调整为 col-date 宽度，实现紧贴日期列 */
    z-index: 10;
