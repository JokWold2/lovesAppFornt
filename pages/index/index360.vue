  ...
  <view class="container">
    ...
    <view class="header-nav">
      <view class="nav-left">
        ...
        <view class="avatar-circle">
          <image v-if="userInfo.avatar_url" class="avatar-img" :src="userInfo.avatar_url" mode="aspectFill"></image>
          <uni-icons v-else type="person-filled" color="#ccc" size="24"></uni-icons>
        </view>
      </view>
    ...
    <view v-if="model === '為您推薦'">
      ...
      <view class="scroll-tabs-wrapper">
        <scroll-view class="scroll-tabs" scroll-x="true" show-scrollbar="false">
          <view class="tabs-content">
            <view class="tab-pill" v-for="(item, index) in originalEntries" :key="index"
              :class="{ 'active': currentEntryIndex === index }" @click="handleEntryClick(index, item.page)">
              <text>{{ item.name }}</text>
            </view>
          </view>
        </scroll-view>
      ...
      <view class="feed-container">
        <!-- 首次加载中 -->
        <view v-if="loading && profiles.length === 0" class="loading-container">
          <text class="loading-text">加载中...</text>
        </view>

        <!-- 空状态 -->
        <view v-if="!loading && profiles.length === 0" class="empty-state">
          <text class="empty-text">暂时没有更多推荐</text>
        </view>

        <view class="post-card" v-for="item in profiles" :key="item.profileId">
          <!-- 帖子头部: 用户信息 -->
          <view class="post-header">
            <view class="post-avatar">
              <image v-if="item.avatarUrl" class="avatar-img" :src="getFullImageUrl(item.avatarUrl)" mode="aspectFill">
              </image>
              <uni-icons v-else type="person-filled" color="#ccc" size="28"></uni-icons>
            </view>
            <view class="post-user-info">
              <text class="username">{{ item.displayName }}</text>
              <view class="location-box">
                <text class="location">{{ formatLocation(item) }}</text>
              </view>
            </view>
          </view>
          ...
          <view class="post-media" v-if="getMainImage(item)">
            <image class="media-img" :src="getMainImage(item)" mode="aspectFill"></image>
          </view>
          ...
          <view class="post-actions">
            <view class="actions-left">
              ...
              <view class="action-btn" @click="toggleCommentPanel(item)">
          ...
          <view v-if="item.showComments" class="meta-box" @click.stop>
            <view v-if="item.commentsLoading" class="comment-loading">
              <text>加载中...</text>
            </view>
            <template v-else>
              ...
              <view v-if="item.comments && item.comments.length === 0" class="comment-empty">
            ...
            <view class="comment-input-row">
              ...
              <view class="comment-input-inner">
                <input class="comment-input" v-model="item.commentDraft" confirm-type="send"
                  :placeholder="item.replyTarget ? `回复 ${item.replyTarget.email}` : '说点什么…'"
        ...
        <view v-if="!hasMore && profiles.length > 0" class="load-more-tip">
          <text>没有更多了</text>
        </view>
      </view>
...
import {
  getExploreFeedApi, toggleProfileLikeApi,
  getProfileCommentsApi, addProfileCommentApi
} from '@/api/index.js';
...
function getMainImage(item) {
  if (item.photos && item.photos.length > 0) return getFullImageUrl(item.photos[1] ? item.photos[1] : item.photos[0]);
  if (item.avatarUrl) return getFullImageUrl(item.avatarUrl);
  return '';
}

function formatLocation(item) {
  return [item.country, item.region].filter(Boolean).join(' · ') || (item.occupation || '');
}
...
async function loadFeed({ isRefresh }) {
  ...
  try {
    const excludeIds = isRefresh ? [] : profiles.value.map(p => p.profileId);
...
function toggleCommentPanel(item) {
  ...
  if (item.showComments && item.comments.length === 0 && !item.commentsLoading) {
    loadComments(item);
  }
}
...
async function submitComment(item) {
  ...
  if (!text) return;
  try {
    const res = await addProfileCommentApi(item.profileId, text, item.replyTarget?.userId);
