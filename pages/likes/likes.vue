<template>
  <view class="likes-page app-h5-min-screen liquid-tab-page">
    ...
    <view v-else-if="!feed.items.length" class="state empty-state"><image src="/static/brand/mutual-hearts.svg" class="empty-heart" mode="aspectFit" /><text class="empty-title">{{ direction === 'incoming' ? t('membership.emptyIncoming') : t('membership.emptyOutgoing') }}</text><text>{{ direction === 'incoming' ? t('membership.emptyIncomingHint') : t('membership.emptyOutgoingHint') }}</text><button @click="goHome">{{ t('membership.browse') }}</button></view>
    <view v-else class="likes-grid">
      <view v-for="(item, index) in feed.items" :key="item.rowKey || item.profileId || index" class="person-card" :class="{ locked: isLocked(item) }" @click="openPerson(item)">
        ...
        <image v-if="!isLocked(item)" class="relationship-heart" :src="item.mutual ? '/static/brand/mutual-hearts.svg' : '/static/brand/single-heart.svg'" :aria-label="item.mutual ? t('membership.mutual') : t('membership.oneSided')" mode="aspectFit" />
