...
test('消息页搜索框从右向左展开，并将标题移动到下一行', async () => {
  ...
  assert.match(source, /transition:\s*width\s+\d+ms cubic-bezier/)
...
test('搜索按钮仅在展开时显示输入框背景色', async () => {
  ...
  assert.match(source, /\.search-open \.search-shell\s*\{[^}]*background:\s*#f8f7f5/)
})
...
test('消息页搜索会筛选互动消息、待审核项和群聊，并提供空状态', async () => {
  ...
  assert.match(source, /t\('inbox.noMatching'\)/)
})
