<template>
  <view class="message-row" :class="{ mine }">
    ...
    <view class="message-main">
      ...
      <view v-if="mine" class="receipt"><view v-if="message.readCount" class="read-summary"><view class="read-avatars"><view v-for="reader in message.readBy.slice(0, 3)" :key="reader.userId" class="read-avatar-wrap"><image v-if="reader.avatarUrl" class="read-avatar" :src="reader.avatarUrl" mode="aspectFill" /><text v-else class="read-avatar read-avatar-fallback">{{ String(reader.name || '成').slice(0, 1) }}</text></view></view><text class="receipt-text">{{ message.readCount }}人已读</text></view><text v-else>未读</text></view>
    </view>
  </view>
</template>
...
.sender-name { margin: 0 8rpx 8rpx; color: #777b83; font-size: 24rpx; }.bubble { max-width: 100%; padding: 18rpx 22rpx; border-radius: 22rpx; background: #fff; box-sizing: border-box; }.message-text { color: #171b29; font-size: 32rpx; line-height: 1.5; word-break: break-word; }.mention { color: #075aa5; font-weight: 600; }.message-image { display: block; width: 360rpx; max-width: 100%; border-radius: 14rpx; }.receipt { min-height: 30rpx; padding: 4rpx 8rpx 0; color: #8c939d; font-size: 21rpx; }.read-summary { display: flex; align-items: center; }.read-avatars { display: flex; padding-left: 4rpx; }.read-avatar-wrap { display: flex; width: 26rpx; height: 26rpx; margin-left: -4rpx; border: 2rpx solid #efefef; border-radius: 50%; overflow: hidden; background: #c6cbd2; }.read-avatar { width: 100%; height: 100%; border-radius: 50%; }.read-avatar-fallback { display: flex; align-items: center; justify-content: center; color: #fff; font-size: 15rpx; }.receipt-text { margin-left: 7rpx; }
