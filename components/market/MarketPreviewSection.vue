<template>
  <view class="market-section">
    ...
    <SlideUpPanel fixed :open="filterOpen" :z-index="300" :label="t('marketSearch.filter')" @dismiss="filterOpen = false">
      <view class="filter-sheet"><view class="handle"/><view class="sheet-title"><text>{{ t(category === 'antique' ? 'marketSearch.antique' : 'marketSearch.secondHand') }}</text><view @click="filterOpen = false"><uni-icons type="closeempty" size="24"/></view></view>
        <view class="search sheet-search"><uni-icons type="search" size="21" color="#888"/><input v-model="draftKeyword" :placeholder="t(category === 'antique' ? 'marketSearch.antique' : 'marketSearch.secondHand')" maxlength="100" confirm-type="search" @confirm="applyFilters"/></view>
...
function applyFilters() {
  ...
  keyword.value = draftKeyword.value.trim(); minPrice.value = min; maxPrice.value = max; sort.value = draftSort.value; filterOpen.value = false; search()
}
...
onBackPress(() => { if (props.active && filterOpen.value) { filterOpen.value = false; return true } })
